      ...
      use local_arrays, only: q2,q3,q1,pr,dens,co2 !,isbody
      ...
      !allocate(prx(n2m,n3m))
      allocate(v1(n2m,n3m))
      allocate(v2(n2m,n3m))
      allocate(v3(n2m,n3m))
      allocate(densx(n2m,n3m))
      allocate(dsalx(n2m,n3m))
      allocate(co2xx(n2m,n3m))
      ...
      do kc=kstart,kend
       ...
       do jc=1,n2m                                                    
        ...
        densx(jc,kc) = dens(ic,jc,kc)
        co2xx(jc,kc) = co2(ic,jc,kc)
      ...
      call h5pcreate_f(H5P_DATASET_XFER_F, plist_id, hdf_error) 
      call h5pset_dxpl_mpio_f(plist_id, H5FD_MPIO_COLLECTIVE_F, hdf_error)
      call h5dwrite_f(dset_den, H5T_NATIVE_DOUBLE, densx(1:n2m,kstart:kend), dims,  hdf_error, file_space_id = filespace, mem_space_id = memspace, xfer_prp = plist_id)
      ...
      call h5pcreate_f(H5P_DATASET_XFER_F, plist_id, hdf_error) 
      call h5pset_dxpl_mpio_f(plist_id, H5FD_MPIO_COLLECTIVE_F, hdf_error)
      call h5dwrite_f(dset_co2, H5T_NATIVE_DOUBLE, co2xx(1:n2m,kstart:kend), dims,  hdf_error, file_space_id = filespace, mem_space_id = memspace, xfer_prp = plist_id)
...
!      write(45,'("</Attribute>")')
     ...
     & AttributeType=""Scalar"" Center=""Node"">")')
      write(45,'("<DataItem Dimensions=""",i4," ",i4,"""&
     & NumberType=""Float"" Precision=""4"" Format=""HDF"">")')n3m,n2m
      write(45,'("frame_",i5.5,"_xcut.h5:/Den")') itime
      write(45,'("</DataItem>")')
      write(45,'("</Attribute>")')
     ...
     & AttributeType=""Scalar"" Center=""Node"">")')
      write(45,'("<DataItem Dimensions=""",i4," ",i4,"""&
     & NumberType=""Float"" Precision=""4"" Format=""HDF"">")')n3m,n2m
      write(45,'("frame_",i5.5,"_xcut.h5:/CO2")') itime
      write(45,'("</DataItem>")')
      write(45,'("</Attribute>")')
